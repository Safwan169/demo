...
/**
 * Read-only picker options for the Stock Journal editor + filter bar. Thin local bindings
 * to the MAS/AUD list endpoints rather than importing `features/master-data` (skill §2.4
 * import boundary). Company implicit from the JWT; the backend scopes project-scoped
 * readers to assigned projects. Godowns/purposes are project-scoped lookups.
 */
...
export async function createPurpose(projectId: string, name: string): Promise<PurposeOption> {
  const res = await apiClient.post<{ data: PurposeOption }>(`/masters/projects/${projectId}/purposes`, { name }, {
